...
def main() -> None:
    ...
    st.info(
        "使用說明：一般情況下，直接用下方「關鍵字搜尋」即可，會同時搜尋代號、公司、簡介、子分類與 AI 標籤。"
        "只有在你想從大量候選中精準挑選固定詞時，才需要使用「子分類候選搜尋」或「AI標籤候選搜尋」來縮小選項。"
        "如果結果仍然太多，再搭配大分類、交易所、最終子分類或 AI 標籤進一步篩選。"
    )
    ...
    with tabs[0]:
        ...
        def _to_tv_symbol(row: pd.Series) -> str:
            ...
            if "NASDAQ" in ex:
                ...
                p = "NYSE"
            elif "AMEX" in ex or "ARCA" in ex:
                p = "AMEX"
            else:
                p = ex.replace(" ", "") if ex else "NASDAQ"
            return f"{p}:{tk}" if tk else ""

        export_df["tradingview_symbol"] = export_df.apply(_to_tv_symbol, axis=1)
        export_df = export_df.rename(columns={"tags": "ai_tags", "summary": "summary_zh_tw"})
        export_df = export_df[
            ["tradingview_symbol", "ticker", "exchange", "company_name", "sector", "subsector", "ai_tags", "summary_zh_tw"]
        ]
        ...
        with pd.ExcelWriter(xbuf, engine="openpyxl") as writer:
            export_df.to_excel(writer, index=False, sheet_name="watchlist")
        ex2.download_button(
            "匯出篩選結果 Excel",
            xbuf.getvalue(),
