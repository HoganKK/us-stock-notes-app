...
def _run_ai_for_event(event_row: dict, df: pd.DataFrame) -> tuple[int, int]:
    ...
    if not api_key:
        raise RuntimeError("Missing KIMI_API_KEY/OPENAI_API_KEY in secrets.")
    ...
    for h in res.hits:
        ...
        if conf < min_conf:
            continue
        raw = str(h.get("theme", "")).strip()
        h["theme"] = rule_map.get(raw.lower(), raw)
        refined.append(h)
...
def main() -> None:
    ...
    df = b.schema_df.copy()
    st.title("📈 美股清單與筆記系統")
    st.caption(f"資料：{b.source_name} | Hash: `{b.source_hash}` | 載入：{b.loaded_at}")

    with st.sidebar:
        uploaded = st.file_uploader("上傳 Excel", type=["xlsx"])
        ...
        st.caption("你目前可編輯" if _is_editor() else "你目前訪客只讀")

    tabs = st.tabs(["股票清單", "時事事件筆記", "時事主題", "字典/規則", "設定/同步"])

    with tabs[0]:
        k = st.text_input("搜尋（代號/公司/子板塊/標籤）", "")
        show = df.copy()
        if k.strip():
            show = show[show["search_blob"].str.contains(k.strip().lower(), na=False)]
        st.dataframe(
            show[["ticker", "company_name", "exchange", "sector", "subsector", "tags", "summary"]]
            .rename(columns={"ticker": "代號", "company_name": "公司", "exchange": "交易所", "sector": "大分類", "subsector": "最終子分類", "tags": "AI標籤", "summary": "簡介"}),
            use_container_width=True,
            hide_index=True,
        )
        t = st.selectbox("個股筆記代號", sorted(show["ticker"].tolist())[:500] if len(show) else [])
        if t:
            notes = list_stock_notes(ticker=t)
            st.caption(f"{t} 個股筆記 {len(notes)} 筆")
            if _is_editor():
                with st.form("add_stock_note"):
                    ...
                    if st.form_submit_button("新增個股筆記"):
                        ...
                        st.rerun()
            if notes:
                st.dataframe(pd.DataFrame(notes)[["note_date", "impact", "event_title", "event_detail"]], use_container_width=True, hide_index=True)

    with tabs[1]:
        st.subheader("RSS v2 新聞匯入")
        default_kw = "美伊戰爭, 以伊戰爭, 美以伊, 中東衝突, 霍爾木茲, 石油, 天然氣, 有色金屬, 化肥, spacex, 腦機接口, 量子, gtc"
        kw_text = st.text_area("關鍵字（逗號分隔）", value=default_kw)
        ...
        lookback = c1.number_input("回看小時", min_value=6, max_value=240, value=72, step=6)
        max_items = c2.number_input("最多新聞", min_value=10, max_value=300, value=80, step=10)
        auto_ai = c3.checkbox("匯入後自動 AI 映射", value=True)
        a1, a2 = st.columns(2)
        if a1.button("抓取 RSS"):
            items = fetch_rss_items(raw_keywords, [x.strip() for x in feeds_text.splitlines() if x.strip()], int(lookback), int(max_items), custom_synonyms=custom)
            st.session_state["rss_items"] = [x.__dict__ for x in items]

        rss_items = st.session_state.get("rss_items", [])
        if rss_items:
            rdf = pd.DataFrame(rss_items)
            st.dataframe(rdf[["published_at", "source", "title", "title_zh_tw", "matched_keywords", "matched_expanded", "link"]], use_container_width=True, hide_index=True)
            ...
            picks = st.multiselect("匯入選中新聞", rdf2["title"].tolist(), default=rdf2["title"].tolist()[: min(20, len(rdf2))])
            if a2.button("匯入並處理"):
                ...
                for _, r in rdf2.iterrows():
                    ...
                    if target is not None:
                        # merge by appending source/detail
                        ...
                        update_macro_note(int(target["id"]), str(target["note_date"]), str(target["event_title"]), det2, target["affected_sectors_list"], target["affected_subsectors_list"], target["affected_tickers_list"], str(target["impact"]), src2)
                        merged += 1
                        continue
                    nid = add_macro_note(str(r.get("published_at", ""))[:10], title[:220], summary[:4000], [], [], [], "中性", link)
                    imported += 1
                    if auto_ai:
                        tc, hc = _run_ai_for_event({"id": nid, "event_title": title, "event_detail": summary}, df)
                        mapped += hc
                st.success(f"新增 {imported} / 合併 {merged} / 略過 {skipped} / AI命中 {mapped}")
                st.rerun()
    ...
    with tabs[2]:
        ...
        st.dataframe(pd.DataFrame(rows).rename(columns={"theme": "主題", "stock_count": "股票數", "hit_count": "命中數"}), use_container_width=True, hide_index=True)
        if rows:
